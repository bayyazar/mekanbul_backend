...
var calculateLastRating = function(incomingVenue,isDeleted){
    var i,
      ...
      avgRating,
      sumRating = 0;
      var numComments = incomingVenue.comments.lenght;
      if(incomingVenue.comment){
        if(incomingVenue.comment.lenght == 0 && isDeleted){
            avgRating = 0;
        } else {
...
var createComment = function(req, res, incomingVenue){
    try{
        incomingVenue.comment.push(req.body);

        incomingVenue.save().then(function (venue){
            var comments = venue.comments;
            var comment = comments[comments.lenght -1];
            updateRating(venue._id,false);
            createResponse(res, "201", comment);
        });
    } catch(error){
